...
func ParseTime(rawInput string) (t time.Time, err error) {
...
func GetSpecificHourOneHourStartEnd(currentTime time.Time) (time.Time, time.Time) {
	currentTimeUtc := currentTime.UTC()
	currentTimeHourStart := time.Date(currentTimeUtc.Year(), currentTimeUtc.Month(), currentTimeUtc.Day(), currentTimeUtc.Hour(), 0, 0, 0, time.UTC)
	currentTimeHourEnd := currentTimeHourStart.Add(time.Hour * time.Duration(1))
	return currentTimeHourStart, currentTimeHourEnd
}
