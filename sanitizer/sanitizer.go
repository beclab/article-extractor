	...
	for {
		...
		case html.SelfClosingTagToken:
			...
			if !isPixelTracker(tagName, token.Attr) && isValidTag(tagName) {
				...
				if hasRequiredAttributes(tagName, attrNames) || (tagName == "img" && parentTag == "picture") {
					if len(attrNames) > 0 {
						buffer.WriteString("<" + tagName + " " + htmlAttributes + "/>")
					} else {
						buffer.WriteString("<" + tagName + "/>")
...
func getAttribute(attributes []html.Attribute, attrName string) string {
...
func sanitizeAttributes(baseURL, tagName string, attributes []html.Attribute) ([]string, string) {
	...
	if tagName == "img" {
		...
		srcsetVal := getAttribute(attributes, "srcset")
		if srcsetVal != "" {
			isSrcsetImg = true
		}
		/*srcVal := getAttribute(attributes, "src")

		if srcVal == "" && srcsetVal != "" {
			srcsetImgChangeSrc = sanitizeSrcsetSrcAttr(baseURL, srcsetVal)
		}*/
	}
	...
	for _, attribute := range attributes {
		...
		/*if srcsetImgChangeSrc != "" {
			if attribute.Key == "srcset" {
				continue
			}
			if attribute.Key == "src" || attribute.Key == "data-src" {
				value = srcsetImgChangeSrc
			}
		}*/

		if isSrcsetImg && (attribute.Key == "width" || attribute.Key == "height") {
			continue
		}
...
func sanitizeSrcsetSrcAttr(baseURL, value string) string {
	imageCandidates := ParseSrcSetAttribute(value)
	srcsetLen := len(imageCandidates)
	...
	return imageCandidates[srcsetLen-1].ImageURL
}
